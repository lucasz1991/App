...
- Reverb-Nachrichten verwenden bei einem Zugriff von einem anderen Rechner automatisch den tatsächlich geöffneten RailTime-Host statt dessen lokales `localhost`. Der Reverb-Server lauscht dafür auf allen Netzwerkschnittstellen; bei einem Verbindungsabbruch aktualisiert der Chat spätestens nach zwei statt bisher fünf Sekunden.
...
- Der Hinweis zur nicht erneuten Wiedergabe erscheint ausschließlich bei tatsächlich verbrauchten Einmal-Sprachnachrichten. Dauer, Balkenanzahl und Fortschrittsanimation des Players folgen der gespeicherten beziehungsweise erkannten Audiodauer und enden synchron mit der Wiedergabe.
- Aktive Untermenü-Icons werden bei eingeklappter Desktop-Sidebar trotz ihres offenen Einzugs exakt innerhalb der schmalen Sidebar zentriert und liegen nicht mehr zu nah am Rand.
