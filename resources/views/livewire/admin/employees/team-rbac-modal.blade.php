    ...
    <x-slot name="content">
        <div class="space-y-4">
            ...
            <div class="grid md:grid-cols-[220px_1fr] gap-4">
                <div class="border border-slate-200 rounded-xl p-2 max-h-[440px] overflow-y-auto bg-slate-50 dark:bg-slate-800/50 dark:border-slate-700">
                    @forelse($teams as $team)
                        <button
                            type="button"
                            wire:click="setTeam({{ $team->id }})"
                            class="w-full text-left px-3 py-2 rounded-lg mb-1 text-sm transition {{ (int)$selectedTeamId === (int)$team->id ? 'bg-rt-red text-white font-semibold' : 'bg-white hover:bg-slate-50 hover:text-rt-red text-slate-700 border border-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 dark:border-slate-600 dark:hover:text-rt-red' }}"
                        >
                            {{ $team->name }}
                        </button>
                    @empty
                        <div class="text-xs text-slate-500 p-2 dark:text-slate-400">{{ __('app.no_teams') }}</div>
                    @endforelse
                </div>

                <div class="border border-slate-200 rounded-xl p-3 bg-white dark:bg-slate-800 dark:border-slate-700">
                    @if($selectedTeamId)
                        ...
                        <div class="font-semibold text-slate-800 mb-2 dark:text-slate-100">{{ __('app.permissions') }}</div>
                        <div class="border border-slate-100 rounded-lg p-3 mb-3  max-h-[440px] overflow-y-auto scroll-container dark:border-slate-700">
                            <div class="grid grid-cols-1 gap-3">
                                @foreach($permissionGroups as $groupLabel => $permissions)
                                    <div class="border border-slate-100 rounded-lg p-2 dark:border-slate-700">
                                        <div class="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2 dark:text-slate-400">{{ $groupLabel }}</div>
                                        <div class="space-y-1">
                    ...
                    @else
                        <div class="text-sm text-slate-500 dark:text-slate-400">{{ __('app.select_team_first') }}</div>
                    @endif
                </div>
            </div>
        </div>
