<div class="ops-stack">
    ...
    @if($module === 'rules')
        <div class="ops-split">
            ...
            <form wire:submit="saveRules" class="ops-panel ops-form"><h2 class="ops-full">Neues Regelprofil</h2><x-operations.field label="Bezeichnung" model="rules.name" required :wide="true" />
                ...
                <label class="ops-check ops-full"><input type="checkbox" wire:model="rules.confirmed" required>Betrieblich geprüfte Regeln freigeben</label>
    ...
    @else
        <div class="ops-toolbar"><div class="ops-field"><input type="search" aria-label="Mitarbeiter suchen" placeholder="Mitarbeiter suchen" wire:model.live.debounce.300ms="search"></div><div class="ops-field"><select wire:model.live="filter" aria-label="Prüfstatus"><option value="pending">In Prüfung</option><option value="approved">Freigegeben</option><option value="all">Alle</option></select></div></div>
        @if($module === 'qualifications')<details class="ops-panel"><summary>Nachweisarten · {{ $types->count() }}</summary><div class="ops-actions">@foreach($types as $type)<span class="ops-badge">{{ $type->name }}</span>@endforeach</div><form wire:submit="addType" class="ops-row"><x-operations.field label="Neue Nachweisart" model="typeName" required maxlength="180" /><x-ui.buttons.button-basic type="submit">Hinzufügen</x-ui.buttons.button-basic></form></details>@endif
        <section aria-label="Prüfliste">
            @forelse($records as $record)
                <article class="ops-panel ops-stack" wire:key="personnel-{{ $record->id }}"><header class="ops-toolbar"><div><p class="ops-kicker">{{ $record->user->name }} · Revision {{ $record->revision }}</p><h2>{{ $module === 'qualifications' ? $record->type->name : ['vacation'=>'Urlaub','unavailable'=>'Nicht verfügbar','other'=>'Abwesenheit'][$record->kind] }}</h2></div><x-operations.status :value="$record->status" /></header>
            ...
            @empty<div class="ops-panel ops-empty">Keine Einträge in dieser Ansicht.</div>@endforelse
            {{ $records->links() }}
        </section>
    @endif
</div>
