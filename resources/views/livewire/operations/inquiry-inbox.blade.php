<div class="ops-stack">
    <div class="ops-toolbar">
        <div class="ops-actions"><div class="ops-field"><input type="search" wire:model.live.debounce.300ms="search" placeholder="Anfrage oder Kunde suchen" aria-label="Anfragen suchen"></div><div class="ops-field"><select wire:model.live="filter" aria-label="Anfragen filtern"><option value="active">Offene Vorgänge</option><option value="all">Alle Vorgänge</option><option value="email">E-Mail</option><option value="phone">Telefon</option><option value="portal">Portal</option><option value="manual">Manuell</option></select></div></div>
        <x-ui.buttons.button-basic mode="primary" wire:click="create">Anfrage erfassen</x-ui.buttons.button-basic>
    </div>
    <x-operations.feedback />
    <div @class(['ops-split ops-inbox', 'has-selection' => $selected || $editing])>
        <section class="ops-panel ops-inbox-list" aria-label="Anfrageliste">
            @forelse($inquiries as $inquiry)
                <button type="button" class="ops-list-choice" wire:click="select({{ $inquiry->id }})" aria-pressed="{{ $selectedId === $inquiry->id ? 'true' : 'false' }}" wire:key="inquiry-{{ $inquiry->id }}">
                    <span class="ops-toolbar"><span class="ops-muted">{{ $inquiry->number }} · {{ ['email'=>'E-Mail','phone'=>'Telefon','portal'=>'Portal','manual'=>'Manuell'][$inquiry->channel] }}</span><x-operations.status :value="$inquiry->status" /></span>
                    <strong>{{ $inquiry->title }}</strong><span class="ops-muted">{{ $inquiry->customer?->company_name ?? 'Kunde offen' }}</span>
                </button>
            @empty<div class="ops-empty">Keine Anfragen.</div>@endforelse
            {{ $inquiries->links() }}
        </section>
        <section class="ops-panel ops-stack" aria-label="Anfragedetails">
            @if($selected || $editing)
                <x-ui.buttons.button-basic class="ops-mobile-back" wire:click="close">Zur Liste</x-ui.buttons.button-basic>
            @endif
            @if($editing)
                <h2>{{ $selectedId ? 'Bedarf bearbeiten' : 'Neue Anfrage' }}</h2>
                <form wire:submit="save" class="ops-form">
                    ...
                    <div class="ops-full ops-actions"><x-ui.buttons.button-basic type="submit" mode="primary" wire:loading.attr="disabled">Speichern</x-ui.buttons.button-basic><x-ui.buttons.button-basic wire:click="$set('editing', false)">Abbrechen</x-ui.buttons.button-basic></div>
                </form>
            @elseif($selected)
                ...
                @if(!$selected->order_id && !$selected->duplicate_of_id)
                    ...
                    @if($selected->status === 'offered')
                        <form wire:submit="transition('accept')" class="ops-stack"><x-operations.field label="Kundenzusage · Person, Zeitpunkt, Referenz" model="acceptance" type="textarea" required /><label class="ops-check"><input type="checkbox" wire:model="authorized" required>Beauftragung durch berechtigten Kundenkontakt bestätigt</label><x-ui.buttons.button-basic type="submit" mode="primary" wire:loading.attr="disabled">Zusage dokumentieren</x-ui.buttons.button-basic></form>
