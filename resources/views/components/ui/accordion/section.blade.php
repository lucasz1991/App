@props([
    ...
    'idPrefix' => 'rt-accordion',
])

@php
    $sectionKey = \Illuminate\Support\Str::slug((string) $section);
    $prefix = \Illuminate\Support\Str::slug((string) $idPrefix);
    $triggerId = $prefix.'-'.$sectionKey.'-trigger';
    $panelId = $prefix.'-'.$sectionKey.'-panel';
@endphp
...
<section
    ...
    data-rt-accordion-section="{{ $sectionKey }}"
>
    <h2>
        <button
            id="{{ $triggerId }}"
            type="button"
            x-on:click="toggleAccordionSection(@js($sectionKey))"
            x-bind:aria-expanded="(openAccordionSection === @js($sectionKey)).toString()"
            ...
            x-bind:class="openAccordionSection === @js($sectionKey) && 'bg-rt-accent-soft/45 dark:bg-rt-dark-accent-soft/35'"
        >
            ...
            <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-rt-surface-muted text-rt-muted transition duration-200 group-hover:text-rt-accent dark:bg-rt-dark-surface-muted dark:text-rt-dark-muted dark:group-hover:text-rt-dark-accent">
                <i
                    ...
                    x-bind:class="openAccordionSection === @js($sectionKey) && 'rotate-180'"
                    aria-hidden="true"
                ></i>
            </span>
        </button>
    ...
    <div
        id="{{ $panelId }}"
        x-show="openAccordionSection === @js($sectionKey)"
