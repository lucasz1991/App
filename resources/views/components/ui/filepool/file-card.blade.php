...
<div    x-data="{ isHovered: false }"
    ...
    <div class="absolute inset-0 flex items-center justify-center flex-wrap bg-rt-surface/70 dark:bg-rt-dark-canvas/75 rounded-xl" x-show="isHovered" x-collapse>
        <button
                ...
                class="text-slate-600 hover:text-rt-red dark:text-slate-200 dark:hover:text-rt-red text-sm bg-slate-200 dark:bg-slate-600 rounded-full p-2 m-2 transition focus:outline-none focus:ring-2 focus:ring-rt-red/40">
        ...
        @if(!$readOnly && $file->is_owned_by_auth_user)
            ...
            <button wire:click="deleteFile({{ $file->id }})"
                    ...
                    title="{{ __('app.delete') }}"
                    class="text-red-600 hover:text-red-700 dark:text-red-400 text-sm bg-slate-200 dark:bg-slate-600 rounded-full p-2 m-2 transition focus:outline-none focus:ring-2 focus:ring-rt-red/40">
