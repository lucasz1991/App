...
<div
    {{ $attributes->class('rt-ui-surface rounded-[20px] border border-rt-border bg-rt-surface/90 px-3 py-2.5 text-rt-text transition-all duration-200 ease-out dark:border-rt-dark-border dark:bg-rt-dark-surface/90 dark:text-rt-dark-text') }}
    x-data="{
        field: @js($field),
...
>
    ...
    >
        <span class="flex min-w-0 items-center gap-2.5">
            ...
            <span class="min-w-0">
                <span class="block truncate text-sm font-semibold text-rt-text dark:text-rt-dark-text">{{ $label }}</span>
                <span class="block text-[11px] text-rt-muted dark:text-rt-dark-muted">Inline Picker</span>
            </span>
        </span>

        <span class="flex items-center gap-2">
            <span class="rt-ui-surface-muted rounded-full bg-rt-surface-muted px-2.5 py-1 font-mono text-[11px] text-rt-muted dark:bg-rt-dark-surface-muted dark:text-rt-dark-muted" x-text="value"></span>
            <svg class="h-4 w-4 text-rt-soft transition-transform duration-200 dark:text-rt-dark-soft" :class="{ 'rotate-180': open }" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
    ...
    <div x-show="open" x-collapse.duration.250ms class="overflow-hidden">
        <div class="pt-3">
            @if ($description)
                <p class="text-xs leading-5 text-rt-muted dark:text-rt-dark-muted">{{ $description }}</p>
            @endif

            @if (!empty($usage))
                <div class="mt-2.5 flex flex-wrap gap-2">
                    @foreach ($usage as $item)
                        <span class="rounded-full border border-rt-border px-3 py-1 text-xs text-rt-muted dark:border-rt-dark-border dark:text-rt-dark-muted">{{ $item }}</span>
                    @endforeach
                </div>
            @endif

            <div x-ref="picker" class="rt-ui-surface-muted mt-3 rounded-[18px] border border-rt-border bg-rt-surface-muted/80 p-3 dark:border-rt-dark-border dark:bg-rt-dark-surface-muted/80">
