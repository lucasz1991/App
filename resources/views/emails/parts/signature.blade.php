...
<tr>
    ...
    <td class="rt-pad" bgcolor="{{ $values['SIGNATURE_BG'] }}" style="padding:{{ $padding }};background:{{ $values['SIGNATURE_BG'] }};background-image:url('{{ $values['TRAIN_SRC'] }}');background-repeat:no-repeat;background-position:center bottom;background-size:100% auto;{{ $topRule }}">
        ...
        <table role="presentation" dir="rtl" width="100%" border="0" cellspacing="0" cellpadding="0" style="width:100%;border-collapse:collapse;">
            <tr class="rt-stack">
                <td dir="ltr" class="rt-sign-logo" width="37%" valign="top" align="right" style="direction:ltr;width:37%;padding-left:28px;border-left:1px solid {{ $values['SIGNATURE_BORDER'] }};text-align:right;">
                    ...
                    <p class="rt-only-wide" style="margin:13px 0 0;color:{{ $values['SIGNATURE_META_TEXT'] }};font-size:11px;line-height:18px;"><strong style="color:{{ $values['SIGNATURE_TEXT_PRIMARY'] }};">{{ $values['FIRMENNAME'] }}</strong><br>{{ $values['FIRMENSTRASSE'] }} · {{ $values['FIRMEN_PLZ_ORT'] }}<!-- RT_COMPANY_PHONE_START --><br>T <a href="tel:{{ $values['FIRMEN_TELEFON_TEL'] }}" style="color:{{ $values['SIGNATURE_TEXT_PRIMARY'] }};text-decoration:none;">{{ $values['FIRMEN_TELEFON'] }}</a><!-- RT_COMPANY_PHONE_END --><br><a href="mailto:{{ $values['FIRMEN_EMAIL'] }}" style="color:{{ $values['SIGNATURE_META_TEXT'] }};text-decoration:none;">{{ $values['FIRMEN_EMAIL'] }}</a></p>
                </td>
                <td dir="ltr" class="rt-sign-identity" width="63%" valign="top" align="left" style="direction:ltr;width:63%;padding-right:28px;text-align:left;">
                    ...
                    <p class="rt-only-narrow" style="display:none;max-height:0;overflow:hidden;mso-hide:all;margin:17px 0 0;font-size:0;line-height:0;color:{{ $values['SIGNATURE_META_TEXT'] }};"><strong style="color:{{ $values['SIGNATURE_TEXT_PRIMARY'] }};">{{ $values['FIRMENNAME'] }}</strong><br>{{ $values['FIRMENSTRASSE'] }} · {{ $values['FIRMEN_PLZ_ORT'] }}<!-- RT_COMPANY_PHONE_START --><br>T <a href="tel:{{ $values['FIRMEN_TELEFON_TEL'] }}" style="color:{{ $values['SIGNATURE_TEXT_PRIMARY'] }};text-decoration:none;">{{ $values['FIRMEN_TELEFON'] }}</a><!-- RT_COMPANY_PHONE_END --><br><a href="mailto:{{ $values['FIRMEN_EMAIL'] }}" style="color:{{ $values['SIGNATURE_META_TEXT'] }};text-decoration:none;">{{ $values['FIRMEN_EMAIL'] }}</a></p>
                </td>
            </tr>
        </table>
    </td>
