...
return new class extends Migration
{
    ...
    /**
     * Exakt ausgelieferte, noch unveränderte Kurzfassungen des Premium-
     * Jobmotivs. Eine Variante wird nur aktualisiert, wenn alle drei Formate
     * gemeinsam demselben Release entsprechen; gemischte Releases gelten als
     * individuelle Bearbeitung und bleiben unangetastet.
     *
    ...
    public function up(): void
    {
        ...
        DB::transaction(function () use ($templates, $binder, $sanitizer, $studio): void {
            ...
            foreach ($creatives as $creative) {
                ...
                if (! $this->isUntouchedShortenedJob($creative, $variants, $studio)) {
                    continue;
                }
    ...
    private function isUntouchedShortenedJob(
        ...
        foreach (self::RELEASES as $release) {
            if ($creative->title !== $release['title']
                || ! hash_equals($release['editorial_hash'], $editorialHash)) {
                continue;
            }

            if ($this->matchesCompleteRelease($variants, $release['variants'], $studio)) {
                return true;
            }
        }
    ...
    /**
     * @param  Collection<int, MarketingCreativeVariant>  $variants
     * @param  array<string, array{html:string,css:string}>  $fingerprintsByFormat
     */
    private function matchesCompleteRelease(
        Collection $variants,
        array $fingerprintsByFormat,
        MarketingStudioService $studio,
    ): bool {
        $seenFormats = [];
        ...
        foreach ($variants as $variant) {
            ...
            $fingerprints = $fingerprintsByFormat[$format] ?? null;
